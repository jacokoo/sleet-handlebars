...
const isInDynamic = (stack) => {
    ...
    if (h && h.node.name && !h.note.content)
        return true;
    ...
    if (!tag.name)
        return false;
    if (stack.note('blocks').indexOf(tag.name) !== -1)
        return true;
    if (stack.note('inlines').indexOf(tag.name) !== -1)
        return true;
    if (tag.name.slice(0, 1) === '@' && stack.note('inlines').indexOf(tag.name.slice(1)) !== -1)
        return true;
    return false;
};
...
class IdentifierValueCompiler extends sleet_1.AbstractCompiler {
    ...
    compile(context) {
        const s = this.stack.last(sleet_1.NodeType.Tag);
